...
import { roomRef, playersCol, listenRoom, fetchPlayers, joinRoom, leaveRoom, startMatch, rollDiceTxn, movePieceTxn } from "./state.js";
import { createBoard, renderPlayersList, el, screenToPiecePick } from "./ui.js";
import { COLORS } from "./ludo.js";
...
boardCanvas.addEventListener("click", async (e)=>{
  if (!currentRoom) return;
  // Only act if it's our turn and we have a die value
  ...
  if (!pick) return;
  try {
    await movePieceTxn(currentRoom, uid, pick.pieceIndex);
...
function subscribe(code){
  ...
  unsub = listenRoom(code, async (snap)=>{
    ...
    const amHost = st.order?.length ? st.order[0] === uid : true; // crude host guess (or keep first joiner as host)
    ...
    } else if (st.status==="finished") hud.textContent = "Game over! (Refresh or leave/join to play again)";
